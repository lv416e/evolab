...
public:
    ...
    GAResult<typename P::GenomeT> run(const P& problem, const GAConfig& config = {}) {
        ...
        for (std::size_t gen = 0; gen < config.max_generations; ++gen) {
            ...
            auto gen_best_idx = std::min_element(fitnesses.begin(), fitnesses.end()) - fitnesses.begin();
            ...
            if (gen % config.log_interval == 0) {
                ...
                stats.mean_fitness = Fitness(std::accumulate(fitnesses.begin(), fitnesses.end(), 0.0,
                    [](double sum, const Fitness& f) { return sum + f.value; }) / fitnesses.size());
    ...
    double calculate_diversity(const std::vector<GenomeT>& population) {
        ...
        for (std::size_t i = 0; i < population.size(); ++i) {
            for (std::size_t j = i + 1; j < population.size(); ++j) {
                ...
                double distance = 0.0;
                for (std::size_t k = 0; k < std::min(population[i].size(), population[j].size()); ++k) {
...
auto make_ga(Selection sel, Crossover cross, Mutation mut) {
    return GeneticAlgorithm<Selection, Crossover, Mutation>(std::move(sel), std::move(cross), std::move(mut));
}

template<SelectionOperator Selection, CrossoverOperator Crossover, MutationOperator Mutation, LocalSearchOperator LocalSearch>
auto make_ga(Selection sel, Crossover cross, Mutation mut, LocalSearch ls) {
    return GeneticAlgorithm<Selection, Crossover, Mutation, LocalSearch>(std::move(sel), std::move(cross), std::move(mut), std::move(ls));
}
