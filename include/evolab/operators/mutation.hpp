    ...
    void mutate(const P& problem, typename P::GenomeT& genome, std::mt19937& rng) const {
        ...
        for (std::size_t i = 0; i < weights_.size(); ++i) {
            ...
            if (rand_val <= cumulative) {
                switch (i) {
                    case 0: swap_.mutate(problem, genome, rng); break;
                    case 1: inversion_.mutate(problem, genome, rng); break;
                    case 2: scramble_.mutate(problem, genome, rng); break;
                    case 3: insertion_.mutate(problem, genome, rng); break;
                }
                break;
            }
        }
...
public:
    template<core::Problem P>
    void mutate(const P& problem, typename P::GenomeT& genome, std::mt19937& rng) const {
        if (genome.size() < 4) return;  // Need at least 4 cities for 2-opt
